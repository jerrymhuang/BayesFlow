...
def test_valid_summaries(approximator_with_summaries, mean_std_summary_network, monkeypatch):
    ...
    approximator_with_summaries.standardizer.build(
        {"inference_variables": (2, 2), "inference_conditions": (2, 3), "summary_variables": (2, 3)}
    )
    ...
    summaries = approximator_with_summaries.summarize({"summary_variables": keras.ops.ones((2, 3))}, batch_size=1)
    assert_allclose(summaries, keras.ops.stack([keras.ops.ones((2,)), keras.ops.zeros((2,))], axis=-1))
